-- Ridgeback booking schema for SPanel / MariaDB 10.11+
-- Create the database and database user in SPanel first, select that database
-- in phpMyAdmin, and then import this file. Do not run this against a database
-- that already contains Ridgeback booking tables.
...
CREATE TABLE IF NOT EXISTS reservations (
  ...
  KEY idx_reservations_user_created (user_id, created_at DESC, id DESC),
  CONSTRAINT fk_reservations_quote FOREIGN KEY (quote_id) REFERENCES quotes (id) ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT fk_reservations_user FOREIGN KEY (user_id) REFERENCES users (id) ON UPDATE RESTRICT ON DELETE SET NULL,
...
INSERT INTO site_types (code, name, description, max_guests_per_site, default_nightly_cents, is_active) VALUES
  ('standard', 'Full Hookup Site', '50-foot concrete pad with 20/30/50A full hookups.', 6, 6500, 1),
  ('premium', 'Big-Rig Premium', 'Retired staging-only site type.', 6, 7900, 0),
  ('monthly', 'Extended Stay', 'Retired staging-only site type.', 6, 2500, 0)
...
SELECT st.id, seed.site_code, IF(st.code = 'standard', 1, 0), 1
...
INSERT INTO schema_migrations (version, checksum) VALUES
  ('001_initial_schema.sql', 'dcd0d2ab6d0e640f3eec9b87a045e0cc967adbf8b693e8745b4dd145bf6688d2'),
  ('002_nullable_reservation_hold.sql', '87cdfad7706aef5bc262e6ddb75c7d29f8683f45cfedbdcbdf38315c25539af4'),
  ('003_nullable_rv_details.sql', '78f4474236666ed4a219c2f14d96241662d63a93b7c936550deb93ed94fd3813'),
  ('004_auth_system.sql', '2a25d4aee7083d82b819e7682fd7e9e0f4e0e24ec912d33a92b36f8e271a72a1')
ON DUPLICATE KEY UPDATE checksum = VALUES(checksum);

-- Verification result sets shown by phpMyAdmin after a successful import.
SELECT VERSION() AS database_version;
SELECT COUNT(*) AS ridgeback_table_count
FROM information_schema.tables
WHERE table_schema = DATABASE();
SELECT COUNT(*) AS site_type_count FROM site_types;
SELECT COUNT(*) AS site_count FROM sites;
SELECT COUNT(*) AS extra_count FROM extras;
